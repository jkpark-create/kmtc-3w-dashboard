...
QUARTER_KEYS = tuple(QUARTER_MONTHS.keys())
...
def attach_missing_quarter_progress(rows: list[dict[str, Any]], month_metrics: dict[str, Any]) -> None:
    """Fill missing perform/progress fields from generated monthly metrics.

    Workbook values win when present. This mainly activates Q3, where Summary_All
    currently provides target columns but no quarter-progress columns.
    """
    for row in rows:
