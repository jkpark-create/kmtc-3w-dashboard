...
- **전체기간(월별)**: 코드에서 '전체기간'은 특정 `YYYYMM`에 해당하는 '그 달의 전체 부킹'을 의미합니다. 예: 5월이면 5월에 속한 모든 부킹이 집계 대상입니다. (구현 참조: `all_month_avg`, `por_month_avg`는 `yyyymm`으로 그룹화합니다.)
- **비교 단위(부킹 단위)**: 고수익/저수익 판정은 '부킹 단위'로 계산한 화주별 CM1/TEU(`s_avg`)를 같은 루트(선적지 `POR`+`DLY`)의 루트 평균(`r_avg`) 또는 동월 전체 화주 평균(`a_avg`)과 비교하여 결정합니다. 즉, 비교는 개별 화주(부킹 단위) 대 루트/동월 전체 평균 간의 비교입니다.
- **혼동 방지 권고 문구**: 문서나 대시보드 문구에서 "전체기간 화주+루트"와 같은 표현은 피하고, 대신 **'동월 전체 부킹(동월 전체 화주 기준)'**, **'루트별(선적지 POR+DLY) 평균'** 등으로 명확히 표기해 주세요.
- **코드 참조 위치**: `shpr_por_month`, `por_month_avg`, `all_month_avg`, `shpr_all_month` (해당 섹션에서 월별/루트별/전체 월평균 비교 로직을 확인할 수 있음).
