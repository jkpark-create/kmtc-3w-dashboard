...
BKG와 BSA 모두 대시보드의 `dest`를 국가 코드 단위로 유지한다. 과거에는 `MY/SG`, `AE`처럼 일부 도착지를 그룹으로 묶은 AD열 `D_group`을 대시보드 도착지 기준으로 사용했지만, 현재 대시보드의 `dest`는 원본 도착 국가 코드(`DLY_CTR_CD`/`DLY_Country`)를 그대로 사용한다.

`D_group` 컬럼은 기존 산출물 호환을 위해 그룹핑 값으로 유지한다. 즉 `MY`, `SG`는 `MY/SG`, `AE`, `SA`, `KW`, `QA`, `OM`, `BH`, `IQ`, `JO`, `EG`는 `AE`로 묶이고, 기타 국가는 원본 코드가 유지된다. 대시보드 필터/집계는 이 컬럼이 아니라 J열 `DLY_CTR_CD`를 기준으로 생성한다.
...
`fOri`, `fOriP`, `fDst`, `fDstP`는 검색 가능한 체크박스형 멀티셀렉트이며, 선택값이 없으면 전체로 처리한다.

- 멀티셀렉트는 `_selected`와 `_pending` 상태를 분리한다. `_selected`는 현재 대시보드에 적용된 값이고, `_pending`은 메뉴 안에서 아직 적용하지 않은 임시 선택값이다.
- 체크박스 선택/해제와 검색어 입력은 메뉴 내부 상태와 옵션 목록만 갱신한다. 대시보드 cascade/render는 실행하지 않는다.
- `적용` 버튼을 누르면 `_pending`을 `_selected`로 확정하고, `CustomEvent('change', {detail: {source: 'multi-select'}})`를 발생시켜 cascade와 render를 한 번만 실행한다.
- 검색 input의 native `change` 이벤트가 필터 적용으로 오인되지 않도록, 멀티셀렉트 change listener는 `detail.source === 'multi-select'`인 이벤트만 처리한다.
- `전체`는 `_pending`만 빈 배열로 만들고, `닫기`/외부 클릭/ESC는 `_pending`을 `_selected`로 되돌린다. 따라서 적용 전 선택 변경은 취소할 수 있다.
...
- 업로드 대상:
  ...
  - `obt_exception_history.json`: OBT Exception Monitor 속도/요일 기준 history 전체 백업. 날짜별로 늘리지 않고 고정 파일을 갱신한다.
- `upload_to_gdrive()`는 원격 업로드 전에 `dist/data.json`도 같은 summary JSON으로 복사한다. 따라서 Google Drive와 GitHub Pages가 같은 집계 결과를 바라보게 된다.
- 최신 일자(`PUBLISH_LATEST=1`) 실행에서는 `dist/data.json` 복사 직후 `obt-exception-monitor/build_history.py`를 실행해 history를 미리 생성/누적한다. 날짜가 바뀌면 이 단계에서 새 snapshot이 적재되며, 예외 모니터는 화면 로딩 시 이미 생성된 history를 읽는다. 운영상 임시 제외가 필요하면 `SKIP_OBT_HISTORY_BUILD=1`을 사용한다.
- history 생성은 기본적으로 non-fatal이다. 실패하거나 최신 snapshot 검증이 맞지 않으면 경고만 남기고 일일 업데이트는 계속 진행한다. 검증 실패 시 전체 업데이트를 중단해야 하는 테스트/점검 실행에서는 `OBT_HISTORY_STRICT=1`을 사용한다.
- `obt-exception-monitor/history.json`은 Drive 백업용 전체 history이고, `dist/obt-exception-monitor/history.json`은 GitHub Pages 배포용이다. 배포용 파일은 GitHub 100MB 제한을 피하기 위해 기본 `OBT_HISTORY_DEPLOY_MAX_BYTES=90000000` 상한 안에서 오래된 snapshot부터 줄인다. Drive history 업로드만 임시 제외하려면 `SKIP_OBT_HISTORY_DRIVE_UPLOAD=1`을 사용한다.
...
| 루트 멀티셀렉트 적용/검색 | d1505e6 / ca0d807 | 선적지/선적포트/도착지/도착포트 필터를 검색 가능한 멀티셀렉트로 운영, 적용 버튼에서만 cascade/render 실행 |
