...
- 영업사원별 카드는 상태, 영향 TEU, 관리 Gap, 지연구간, 회복/재확보 후보, 대표 구간을 표시한다.
- 지연구간은 리드타임 트렌드 부족과 최근 부킹속도 부족을 합산한 구간 수이며, 0이면 주로 화주 감소/이탈, 3W 약화, BSA 목표 Gap 이슈다.
- 구간별 특이사항은 구간 열을 줄이고 권장 조치 열을 넓혀 조치 문장이 잘 보이도록 조정했다.
- 화주 단위 Action List의 BSA 표기는 화주별 배정이 아니라 Route 전체 기준임을 명확히 하기 위해 `구간BSA`, `구간Gap`으로 변경했다.
