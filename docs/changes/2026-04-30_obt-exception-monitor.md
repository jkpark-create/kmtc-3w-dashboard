...
- 기존 Pages 배포 저장소는 공개 저장소이므로, 회사 Google 계정 전용 접근 조건을 만족하지 않는다.
- 누적 속도 데이터 `obt-exception-monitor/history.json`은 데이터 산출물이므로 공개 저장소 커밋 대상에서 제외했다.
- 실제 보호 배포는 GitHub 소스와 Cloudflare Access, Google Cloud IAP, 또는 Enterprise private/internal Pages 중 하나로 접근 제어를 먼저 구성한 뒤 진행해야 한다.

## 후속 확인사항

- 회사 Google Workspace 도메인 또는 허용 그룹.
- 보호 배포 방식: Cloudflare Access, Google Cloud IAP, GitHub Enterprise private/internal Pages 중 선택.
- 배포 환경에서 `history.json`을 생성/동기화할 위치.
