...
- BKG/BSA 모두 대시보드의 `dest`를 원본 도착 국가 코드 단위로 유지한다.
- `D_group`은 기존 산출물 호환을 위해 기존 그룹핑 값(`MY/SG`, `AE`, 기타 원본 코드)으로 유지한다. 대시보드 `dest`는 `D_group`이 아니라 `DLY_CTR_CD`/`DLY_Country`에서 생성한다.
