...
def cb1(team, ori, ori_p, dst, dst_p, view_mode, month, week):
    ...
    # --- Per-point textposition for scatter (2D overlap avoidance) ---
    _G = 0.07
    _oP, _wP = [], []
    for i in range(len(xl)):
        ...
        res = ['top center', 'bottom center']
        cP, rP = list(bTops), []
        noC = lambda y: not any(abs(y - p) < _G for p in cP)
        noR = lambda y: not any(abs(y - p) < _G for p in rP)
        for e in sorted([e for e in ents if e['yn'] is not None], key=lambda e: -e['yn']):
            td = 0.04 if e['df'] == 'top' else -0.04
            if noC(e['yn'] + td):
                res[e['idx']] = e['df'] + ' center'; cP.append(e['yn'] + td)
            elif noC(e['yn'] - td):
                res[e['idx']] = ('bottom' if e['df'] == 'top' else 'top') + ' center'; cP.append(e['yn'] - td)
            elif noR(e['yn']):
                res[e['idx']] = 'middle right'; rP.append(e['yn'])
            else:
                res[e['idx']] = 'middle left'
    ...
    fb.add_scatter(x=xl, y=w3occ_m, name='3주전실선적/BSA', yaxis='y2', mode='lines+markers+text',
                   ...
                   textposition=_wP, connectgaps=False,
